...
def run_data_fetching_and_processing(params, status_cb=None, progress_cb=None):
    ...
    start_dt_cet = datetime.fromisoformat(params['start_cet']).replace(tzinfo=cet_tz)
    end_dt_cet   = datetime.fromisoformat(params['end_cet']).replace(tzinfo=cet_tz)
    ...
    for entry in all_data:
        if entry.get('dateTimeUtc'):
            cet_dt = datetime.fromisoformat(
                entry['dateTimeUtc'].replace('Z', '+00:00')).astimezone(cet_tz)
...
class App:
    ...
    def _create_tab6_widgets(self):
        ...
        ctrl = ttk.Frame(main, style='Card.TFrame')
        ctrl.pack(fill=tk.X, pady=(0, 4))
        ttk.Label(ctrl, text="Zones (e.g. SE3, SE4):").pack(side=tk.LEFT)
        self.np_zones_entry = ttk.Entry(ctrl, width=36)
        self.np_zones_entry.insert(0, "SE3,SE4")
        self.np_zones_entry.pack(side=tk.LEFT, padx=(4, 16))
    ...
    def _plot_tab5(self):
        ...
        # Show loading state immediately on the canvas
        self._tab5_btn.config(state=tk.DISABLED, text="⏳  Loading…")
        self._tab5_status.config(text="Fetching Nordpool data…")
        self.fig5.clear()
        _ax = self.fig5.add_subplot(111)
        _ax.text(0.5, 0.5, "Fetching Nordpool data, please wait...",
    ...
    def _plot_tab5_thread(self, cnec, area, dtype, data):
        import re as _re
        area_input = area
        country    = _re.sub(r'\d+$', '', area_input).upper()
        ...
        for d_str in dates:
            ...
            try:
                r = requests.get(url,
                                 ...
                                 params={'date': fmt_date, 'locations': country}, timeout=10)
            except Exception:
                continue
            ...
            for e in entries:
                ...
                try:
                    cet_dt = datetime.fromisoformat(
                        ts.replace('Z', '+00:00')).astimezone(cet_tz)
                    key = f"{cet_dt.strftime('%Y%m%d')}_{cet_dt.strftime('%H:%M')}"
                except Exception:
                    continue
                if dtype == "Consumption":
                    np_vals[key] = e.get('volume', 0)
                else:
                    np_vals[key] = (e.get('total', 0) if dtype == "Total"
                                    else e.get('byType', {}).get(api_key, 0))

        self.root.after(0, self._tab5_done,
                        cnec, area_input, country, dtype, data, np_vals, delivery_areas_seen)
    ...
    def _tab5_done(self, cnec, area_input, country, dtype, data,
        ...
        y_ram  = [float(d.get('ram', 0))         for d in data]
        y_sp   = [float(d.get('shadowPrice', 0)) for d in data]
        x_idx  = range(len(xs))
    ...
    # ------------------------------------------------------------------
    def _plot_tab6(self):
        zones = [z.strip() for z in self.np_zones_entry.get().split(',')]
        ...
        self.fig6.clear()
        _ax = self.fig6.add_subplot(111)
        _ax.text(0.5, 0.5, "Fetching Nordpool prices, please wait...",
                 transform=_ax.transAxes, ha='center', va='center',
                 fontsize=12, color=C_MUTED)
        _ax.axis('off')
        self.canvas6.draw()
    ...
    def _plot_tab6_thread(self, zones, dates):
        ...
        for zone in zones:
            ...
            for d_str in dates:
                fmt_date = f"{d_str[:4]}-{d_str[4:6]}-{d_str[6:]}"
                params = {'date': fmt_date, 'areas': zone, 'currency': 'EUR', 'market': 'DayAhead'}
                try:
                ...
                if r.status_code == 200:
                    data = r.json()
                    area_data = next((x for x in data if x.get('deliveryArea') == zone), None)
                    if area_data and 'prices' in area_data:
                        for p in area_data['prices']:
                            p_list.append(p['price'])
                            ts = p.get('deliveryStart', '')
                            try:
                                cet_dt = datetime.fromisoformat(
                                    ts.replace('Z', '+00:00')).astimezone(cet_tz)
                                label = f"{cet_dt.strftime('%m%d')} {cet_dt.strftime('%H:%M')}"
                            except Exception:
                                label = f"{d_str[-4:]} {ts[11:16]}"
                            l_list.append(label)
            series[zone] = (p_list, l_list)

        self.root.after(0, self._tab6_done, zones, series)

    def _tab6_done(self, zones, series, error=None):
        ...
        for i, zone in enumerate(zones):
            ...
            if p_list:
                ...
                ax.plot(range(len(l_list)), p_list, label=zone,
                        color=color, marker='.', markersize=3.5, linewidth=1.4)
                ax.fill_between(range(len(l_list)), p_list, alpha=0.06, color=color)
                found_any = True
    ...
    def _plot_tab7_thread(self, zone, cnec, dates):
        ...
        headers = {'Authorization': f'Bearer {token}', 'accept': 'application/json'}
        net_pos   = {}   # key "YYYYMMDD_HH:MM" CET → MW net position
        price_map = {}   # key "YYYYMMDD_HH:MM" CET → EUR/MWh
        cet_tz = ZoneInfo("Europe/Oslo")

        for d_str in dates:
            fmt = f"{d_str[:4]}-{d_str[4:6]}-{d_str[6:]}"

            # Volumes — deliveryStart is UTC, convert to CET for key matching
            try:
                rv = requests.get(NP_VOL_URL, headers=headers,
                                  params={'market': 'DayAhead', 'areas': zone,
                                          'date': fmt}, timeout=10)
                if rv.status_code == 200:
                    area = next((x for x in rv.json()
                                 if x.get('deliveryArea') == zone), None)
                    if area:
                        for v in area.get('volumes', []):
                            ts = v.get('deliveryStart', '')
                            if ts:
                                cet_dt = datetime.fromisoformat(
                                    ts.replace('Z', '+00:00')).astimezone(cet_tz)
                                key = f"{cet_dt.strftime('%Y%m%d')}_{cet_dt.strftime('%H:%M')}"
                                net_pos[key] = (v.get('sell', 0) or 0) - (v.get('buy', 0) or 0)
            except Exception:
                pass

            # Prices — deliveryStart is also UTC, convert to CET
            try:
                rp = requests.get(NP_PRICE_URL, headers=headers,
                                  params={'date': fmt, 'areas': zone,
                                          'currency': 'EUR',
                                          'market': 'DayAhead'}, timeout=10)
                if rp.status_code == 200:
                    area = next((x for x in rp.json()
                                 if x.get('deliveryArea') == zone), None)
                    if area:
        ...
        self.root.after(0, self._tab7_done, zone, cnec, jao, net_pos, price_map)
    ...
    def _tab7_done(self, zone, cnec, jao, net_pos, price_map, error=None):
        ...
        # Build aligned time series from JAO data as the backbone
        xs     = [f"{str(d['date'])[-4:]} {d['time'][:5]}" for d in jao]
        x_idx  = range(len(xs))
        y_ram  = [float(d.get('ram', 0))         for d in jao]
        y_sp   = [float(d.get('shadowPrice', 0)) for d in jao]
        ...
        ax1b = ax1.twinx()
        ax1b.fill_between(x_idx, y_np, alpha=0.12, color=C_PRIMARY,
                          where=[v >= 0 for v in y_np])
        ax1b.fill_between(x_idx, y_np, alpha=0.12, color=C_RED,
                          where=[v < 0 for v in y_np])
        ax1b.plot(x_idx, y_np, color=C_PRIMARY, linewidth=1.2, label="Net Position (MW)")
        ax1b.axhline(0, color=C_BORDER, linewidth=0.8, linestyle='--')
        ax1b.set_ylabel("Net Position (MW)", color=C_PRIMARY, fontsize=8.5)
        self._style_twin(ax1b, C_PRIMARY)
        ...
        ax2b = ax2.twinx()
        ax2b.fill_between(x_idx, y_np, alpha=0.12, color=C_PRIMARY,
                          where=[v >= 0 for v in y_np])
        ax2b.fill_between(x_idx, y_np, alpha=0.12, color=C_RED,
                          where=[v < 0 for v in y_np])
        ax2b.plot(x_idx, y_np, color=C_PRIMARY, linewidth=1.2, label="Net Position (MW)")
        ax2b.axhline(0, color=C_BORDER, linewidth=0.8, linestyle='--')
        ax2b.set_ylabel("Net Position (MW)", color=C_PRIMARY, fontsize=8.5)
        self._style_twin(ax2b, C_PRIMARY)
        ...
        ax3b = ax3.twinx()
        ax3b.fill_between(x_idx, y_np, alpha=0.12, color=C_PRIMARY,
                          where=[v >= 0 for v in y_np])
        ax3b.fill_between(x_idx, y_np, alpha=0.12, color=C_RED,
                          where=[v < 0 for v in y_np])
        ax3b.plot(x_idx, y_np, color=C_PRIMARY, linewidth=1.2, label="Net Position (MW)")
        ax3b.axhline(0, color=C_BORDER, linewidth=0.8, linestyle='--')
        ax3b.set_ylabel("Net Position (MW)", color=C_PRIMARY, fontsize=8.5)
    ...
    def _run_analysis(self):
        ...
        for d in self.raw_filtered_data:
            if str(d.get('date')) == d_str and d.get('time') == t_str:
                sp     = float(d.get('shadowPrice', 0))
                pf     = float(d.get(f'ptdf_{z_f}', 0))
                pt     = float(d.get(f'ptdf_{z_t}', 0))
                impact = sp * (pf - pt)
                total += impact
                rows.append((d.get('cneName'), d.get('biddingZoneFrom'), d.get('biddingZoneTo'),
                             sp, pf, pt, impact))
    ...
    def _render_histories(self, cnec, z_f, z_t, data):
        xs        = [f"{str(d['date'])[-4:]} {d['time'][:5]}" for d in data]
        x_idx     = range(len(xs))
        sp        = [float(d.get('shadowPrice', 0)) for d in data]
        rams      = [float(d.get('ram', 0))         for d in data]
        fall_data = [float(d.get('fall', 0))         for d in data]
        pf_l      = [float(d.get(f'ptdf_{z_f}', 0)) for d in data]
        pt_l      = [float(d.get(f'ptdf_{z_t}', 0)) for d in data]
    ...
    def _fetch_thread(self, params):
        ...
        # date/time fields already set to CET by the fetch function
    ...
    def _plot_tab8(self):
        ...
        self._t8_status.config(text="Fetching prices & flows...")
        self.fig8.clear()
    ...
    def _plot_tab8_thread(self, date_str, mtu_h, mtu_m):
        ...
        def _to_cet(ts):
            ...
            if s.endswith('Z'):
                ...
                s = s + '+00:00'   # no tz info → assume UTC
            return datetime.fromisoformat(s).astimezone(cet_tz)

        # ── Prices (DayAhead 15-min resolution — match exact date+HH:MM) ──
        prices = {}
        try:
            rp = requests.get(NP_PRICE_URL, headers=headers,
                              params={'date': date_str, 'areas': areas_str,
                                      'currency': 'EUR', 'market': 'DayAhead'},
                              timeout=20)
            if rp.status_code == 200:
                for area_data in rp.json():
        ...
        # ── Scheduled Physical Flows (DayAhead schedule) ──────────────
        flows_raw = {}
        try:
            rf = requests.get(NP_FLOW_URL, headers=headers,
                              params={'market': 'DayAhead', 'date': date_str,
                                      'areas': areas_str},
                              timeout=20)
            if rf.status_code == 200:
                for area_data in rf.json():
                    area      = area_data.get('deliveryArea')
                    flow_list = area_data.get('flows', [])
                    matched   = None
                    for fl in flow_list:
                        ...
                        if ts:
                            try:
                                ...
                                pass
                    if matched:
                        for conn in matched.get('byConnections', []):
                            other = conn.get('area')
                            exp   = float(conn.get('export', 0) or 0)
                            key   = (area, other)
                            flows_raw[key] = flows_raw.get(key, 0) + exp
        except Exception:
            pass

        # Net flow per canonical pair: positive = A→B
        net_flows = {}
        ...
        self.root.after(0, self._tab8_done, prices, net_flows, date_str, mtu_label)

    def _tab8_done(self, prices, net_flows, date_str, mtu_label, error=None):
    ...
    def _ma_fetch_entsoe(self):
        ...
        # Parse CET input and convert to UTC ISO strings for the API
    ...
    def _ma_pipeline_thread(self):
        ...
        try:
            ...
            for v in verdicts:
                m = _re.search(r',\s*p=([0-9.eE+\-]+)', v.get('verdict',''))
    ...
    def _ma_refresh_results(self):
        ...
        for v in verdicts:
            ...
            note    = v.get('text', '')[:80]
            # Parse β and p from the verdict string
            m_coef = _re.search(r'β=([+-]?[0-9.eE+\-]+)', verdict)
            m_pval = _re.search(r',\s*p=([0-9.eE+\-]+)', verdict)
            ...
            if not dep:
                m_dep = _re.search(r'regression for (\S+)', verdict)
    ...
    def _ma_generate_explain(self):
        ...
        if not verdicts:
            ...
            for v in verdicts:
                ...
                verdict = v.get('verdict','')
                note    = v.get('text','')
                m_c = _re.search(r'β=([+-]?[0-9.eE+\-]+)', verdict)
                m_p = _re.search(r',\s*p=([0-9.eE+\-]+)', verdict)
                lines.append(f"\n{hyp}: {verdict}")
                if m_c and m_p:
    ...
    def _ma_export_html(self):
        ...
        def _html_row(v):
            ...
            verdict = v.get('verdict', '')
            note    = v.get('text', '')
            m_dep = _re_html.search(r'regression for (\S+)', verdict)
            dep   = m_dep.group(1) if m_dep else ''
            m_p   = _re_html.search(r',\s*p=([0-9.eE+\-]+)', verdict)
