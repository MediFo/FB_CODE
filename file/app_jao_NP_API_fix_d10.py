...
class App:
    ...
    def _create_tab2_briefing(self, parent):
        """Build the Strategic Market Briefing panel inside Tab 2."""

        # ── Outer layout: narrow left KPI strip + scrollable report ──
        outer = ttk.Frame(parent, style='Card.TFrame')
        outer.pack(fill=tk.BOTH, expand=True)

        # Left KPI strip
        kpi_strip = ttk.Frame(outer, style='Card.TFrame', padding=(12, 14, 8, 12))
        kpi_strip.pack(side=tk.LEFT, fill=tk.Y)

        # Separator line
        sep = tk.Frame(outer, bg=C_BORDER, width=1)
        sep.pack(side=tk.LEFT, fill=tk.Y)

        # Right scrollable report area
        report_outer = ttk.Frame(outer, style='Card.TFrame')
        report_outer.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # ── KPI Strip widgets ─────────────────────────────────────────
        tk.Label(kpi_strip, text="LIVE METRICS", bg=C_PANEL,
                 fg=C_MUTED, font=('Segoe UI', 7, 'bold')).pack(anchor='w', pady=(0, 10))
        ...
        kpi_defs = [
            ...
            ('total_imp',   'Total Impact',       '0.000 €/MWh'),
            ('top_cnec',    'Dominant CNEC',      '—'),
            ('top_sp',      'Shadow Price',       '— €/MWh'),
            ('top_dptdf',   'Delta-PTDF',         '—'),
            ('top_impact',  'CNEC Impact',        '— €/MWh'),
            ('regime',      'Regime',             '—'),
        ]
        for key, label, default in kpi_defs:
            box = tk.Frame(kpi_strip, bg='#f1f5f9', padx=10, pady=7,
                           relief='flat', bd=0)
            box.pack(fill=tk.X, pady=3)
            tk.Label(box, text=label.upper(), bg='#f1f5f9',
                     fg=C_MUTED, font=('Segoe UI', 7, 'bold')).pack(anchor='w')
            var = tk.StringVar(value=default)
            tk.Label(box, textvariable=var, bg='#f1f5f9',
                     fg=C_ACCENT, font=('Segoe UI', 9, 'bold'),
                     wraplength=148, justify='left').pack(anchor='w')
            self._brfg_kpis[key] = var

        tk.Frame(kpi_strip, bg=C_BORDER, height=1).pack(fill=tk.X, pady=10)

        tk.Label(kpi_strip,
                 text="Run Analysis to\nrefresh briefing.",
                 bg=C_PANEL, fg=C_MUTED,
                 font=('Segoe UI', 8), justify='left').pack(anchor='w')

        # ── Scrollable report text area ───────────────────────────────
        report_frame = tk.Frame(report_outer, bg='#0f172a', padx=2, pady=2)
        report_frame.pack(fill=tk.BOTH, expand=True, padx=0, pady=0)
        ...
        _sb = tk.Scrollbar(report_frame, command=self._brfg_text.yview, bg='#1e293b')
        self._brfg_text.config(yscrollcommand=_sb.set)
        ...
        T = self._brfg_text
        T.tag_configure('h1',
            font=('Segoe UI', 11, 'bold'), foreground='#f8fafc',
            spacing3=4)
        T.tag_configure('h2',
            font=('Segoe UI', 9, 'bold'), foreground='#93c5fd',
            spacing1=10, spacing3=2)
        T.tag_configure('h3',
            font=('Segoe UI', 9, 'bold'), foreground='#7dd3fc',
            spacing1=6, spacing3=1)
        T.tag_configure('formula_label',
            font=('Consolas', 8, 'bold'), foreground='#fbbf24',
            spacing1=4)
        T.tag_configure('formula',
            font=('Consolas', 9), foreground='#34d399',
            lmargin1=32, lmargin2=32, spacing1=1, spacing3=1)
        T.tag_configure('body',
            font=('Segoe UI', 9), foreground='#cbd5e1',
            spacing3=2)
        T.tag_configure('body_indent',
            font=('Segoe UI', 9), foreground='#cbd5e1',
            lmargin1=20, lmargin2=20, spacing3=1)
        T.tag_configure('muted',
            font=('Segoe UI', 8), foreground='#64748b',
            spacing3=1)
        T.tag_configure('muted_indent',
            font=('Segoe UI', 8), foreground='#64748b',
            lmargin1=20, lmargin2=20, spacing1=1)
        T.tag_configure('val_neutral',
            font=('Consolas', 9, 'bold'), foreground='#e2e8f0')
        T.tag_configure('val_green',
            font=('Consolas', 9, 'bold'), foreground='#4ade80')
        T.tag_configure('val_amber',
            font=('Consolas', 9, 'bold'), foreground='#fbbf24')
        T.tag_configure('val_red',
            font=('Consolas', 9, 'bold'), foreground='#f87171')
        T.tag_configure('val_blue',
            font=('Consolas', 9, 'bold'), foreground='#60a5fa')
        T.tag_configure('pill_sat',
            font=('Segoe UI', 8, 'bold'), foreground='#0f172a',
            background='#f87171', relief='flat')
        T.tag_configure('pill_struct',
            font=('Segoe UI', 8, 'bold'), foreground='#0f172a',
            background='#fbbf24', relief='flat')
        T.tag_configure('pill_surg',
            font=('Segoe UI', 8, 'bold'), foreground='#0f172a',
            background='#4ade80', relief='flat')
        T.tag_configure('rule',
            font=('Consolas', 8), foreground='#1e3a5f')
        T.tag_configure('tbl_head',
            font=('Consolas', 8, 'bold'), foreground='#93c5fd',
            spacing1=6)
        T.tag_configure('tbl_row_a',
            font=('Consolas', 8), foreground='#94a3b8', background='#0f172a')
        T.tag_configure('tbl_row_b',
            font=('Consolas', 8), foreground='#94a3b8', background='#111827')

        # Populate with placeholder text
        self._brfg_write_placeholder()

    def _brfg_write_placeholder(self):
        """Show static introductory text before any analysis has been run."""
        T = self._brfg_text
        ...
        w("Flow-Based Market Coupling  —  Nordic Domain\n", 'muted')
        w("─" * 72 + "\n", 'rule')
        w("\nNo analysis has been run yet.\n", 'body')
        w("Set your corridor and MTU in the Analysis sub-tab, then click ", 'body')
        w("Run Analysis", 'val_blue')
        w(". The briefing will auto-populate with a full\n"
          "intelligence report for that snapshot.\n", 'body')

        w("\n\nPRIMER  —  HOW THIS BRIEFING WORKS\n", 'h2')
        w("─" * 72 + "\n", 'rule')
        w("\nThis panel translates the raw EUPHEMIA output from the Analysis\n"
          "sub-tab into structured market intelligence. Every number it\n"
          "displays is derived directly from the same JAO shadow-price data\n"
          "you loaded — nothing is estimated or inferred externally.\n\n", 'body')

        w("The core engine behind the Nordic SDAC is EUPHEMIA, a Mixed-\n"
          "Integer Linear Program (MILP) that simultaneously clears all\n"
          "day-ahead auctions across the CWE+Nordic zone. Within that\n"
          "optimisation, each Critical Network Element (CNEC) appears as a\n"
          "linear flow constraint in the feasibility region. When a CNEC is\n"
          "binding — i.e. the optimal power flow would violate it without the\n"
          "constraint — the dual variable of that constraint becomes non-zero.\n"
          "That dual variable is the Shadow Price.\n\n", 'body')

        w("MATHEMATICAL FRAMEWORK  (reference)\n", 'h3')

        w("   [1]  EUPHEMIA primal (simplified)\n", 'formula_label')
        w("        max  SW  =  Σ_z  CS_z  +  PS_z\n"
          "        s.t. NP_z  =  Σ_k  F_k·δ_{k,z}          (nodal balance)\n"
          "             F_i   =  Σ_z  PTDF_{i,z} · NP_z     (linearised flow)\n"
          "             F_i   ≤  RAM_i   ∀ i ∈ CNECs         (capacity)\n"
          "             NP_z  ∈  [NP_z_min, NP_z_max]        (zone bounds)\n\n",
          'formula')

        w("   [2]  Shadow Price  λ_i  (KKT multiplier)\n", 'formula_label')
        w("        λ_i  =  ∂SW* / ∂RAM_i\n"
          "        Interpretation: relaxing CNEC i's RAM by 1 MW increases\n"
          "        total social welfare by λ_i EUR — equivalently, the\n"
          "        constraint costs the market λ_i EUR/MWh of scarcity rent.\n\n",
          'formula')

        w("   [3]  Power Transfer Distribution Factor  PTDF_{i,z}\n", 'formula_label')
        w("        PTDF_{i,z}  =  ∂F_i / ∂NP_z  |_{linearised}\n"
          "        Interpretation: injecting 1 MW net export from zone z\n"
          "        loads PTDF_{i,z} MW onto CNEC i (linearised around the\n"
          "        reference operating point).\n\n",
          'formula')

        w("   [4]  Delta-PTDF for a bilateral corridor  ZF → ZT\n", 'formula_label')
        w("        ΔPTDF_i  =  PTDF_{i,ZF}  −  PTDF_{i,ZT}\n"
          "        Interpretation: transferring 1 MW commercially from ZF\n"
          "        to ZT changes the flow on CNEC i by ΔPTDF_i MW.\n"
          "        Positive → additional commercial flow from ZF to ZT\n"
          "        loads CNEC i further toward its RAM limit.\n\n",
          'formula')

        w("   [5]  Price impact of CNEC i on corridor ZF → ZT\n", 'formula_label')
        w("        PI_i  =  λ_i · ΔPTDF_i  =  λ_i · (PTDF_{i,ZF} − PTDF_{i,ZT})\n"
          "        Units: EUR/MWh  (shadow rent × geometric sensitivity)\n"
          "        Derivation: the market implicitly charges PI_i EUR/MWh\n"
          "        to route any additional MWh across the ZF→ZT corridor,\n"
          "        because doing so would consume ΔPTDF_i MW of CNEC i's\n"
          "        remaining RAM, which is priced at λ_i EUR/MW.\n\n",
          'formula')

        w("   [6]  Total structural price spread  ΔP\n", 'formula_label')
        w("        ΔP  =  P_ZT − P_ZF  =  Σ_i  PI_i  =  Σ_i  λ_i · ΔPTDF_i\n"
          "        This is the number displayed as 'Total Price Impact'\n"
          "        in the Analysis sub-tab and in the KPI strip.\n"
          "        It is the necessary and sufficient explanation of the\n"
          "        price spread arising from the flow-based domain.\n\n",
          'formula')

        w("Run the analysis to see these quantities evaluated for your\n"
          "selected corridor and MTU.\n", 'muted')

        T.config(state='disabled')

    # ------------------------------------------------------------------
    #  TAB 2 – Briefing refresh  (called from _run_analysis)
    # ------------------------------------------------------------------
    def _update_briefing(self, rows, total, d_str, t_str, z_f, z_t):
        """Regenerate the Strategic Briefing report from current analysis results."""
        ...
        top_cne    = top[0]           if top else '—'
        top_sp     = top[3]           if top else 0.0
        top_dptdf  = (top[4] - top[5]) if top else 0.0
        top_impact = top[6]           if top else 0.0

        if top_sp > 50:
            regime = 'SYSTEMIC SATURATION'
        elif top_sp > 10:
            regime = 'STRUCTURAL BOTTLENECK'
        elif top_sp > 0:
            regime = 'SURGICAL BOTTLENECK'
        else:
            regime = 'UNCONGESTED'
        ...
        self._brfg_kpis['top_cnec'].set(top_cne[:30] if top_cne else '—')
        ...
        # ── Helper to colour numeric values ───────────────────────────
        def sp_tag(v):
            if v > 50:  return 'val_red'
            if v > 10:  return 'val_amber'
            if v > 0:   return 'val_green'
            return 'val_neutral'

        def imp_tag(v):
            if abs(v) > 5:  return 'val_red'
            if abs(v) > 1:  return 'val_amber'
            return 'val_green'
        ...
        # ── HEADER ────────────────────────────────────────────────────
        w("STRATEGIC MARKET BRIEFING\n", 'h1')
        w(f"Flow-Based Market Coupling  —  Nordic Domain\n", 'muted')
        w(f"Generated: {ts_now}  |  Analyst layer: EUPHEMIA / JAO Nordic API\n", 'muted')
        w("─" * 72 + "\n", 'rule')
        ...
        direction_word = "positive" if total >= 0 else "negative"
        spread_dir = (f"implying {z_t} clears at a premium relative to {z_f}"
                      if total >= 0
                      else f"implying {z_f} clears at a premium relative to {z_t}")

        w(f"\nFor the MTU ", 'body')
        w(f"{date_fmt}  {mtu_disp} CET", 'val_blue')
        w(f" on the corridor ", 'body')
        w(f"{z_f} \u2192 {z_t}", 'val_blue')
        w(f", the flow-based domain resolved with ", 'body')
        w(f"{n_active}", 'val_amber')
        w(f" active Critical Network Elements (CNECs) carrying a non-zero\n"
          f"shadow rent.\n\n", 'body')

        w("The aggregate structural price spread — computed as the sum of all\n"
          "individual CNEC price impacts on this corridor — is:\n\n", 'body')
        w(f"        ΔP  =  Σ_i  λ_i · (PTDF_{{i,{z_f}}} − PTDF_{{i,{z_t}}})  =  ",
          'formula')
        w(f"{total:+.4f} €/MWh", imp_tag(total))
        w("\n\n", 'formula')
        w(f"This is a {direction_word} spread, {spread_dir}.\n", 'body')

        if n_active == 0:
            w("\nNo binding constraints were found for this MTU. The corridor\n"
              "is operating in an uncongested regime — prices are expected to\n"
              "converge across the bidding zone boundary.\n", 'body')
            T.config(state='disabled')
            return

        # ── SECTION 2: DOMINANT CONSTRAINT ────────────────────────────
        ...
        if top:
            ...
            w(f"\nThe constraint exerting the largest scarcity rent on the\n"
              f"{z_f} \u2192 {z_t} corridor is:\n\n", 'body')
            w(f"  CNEC      :  {cne}\n", 'body_indent')
            w(f"  Zones     :  {czf} \u2192 {czt}\n", 'body_indent')
            w(f"  Shadow Price   λ  :  ", 'body_indent')
            w(f"{sp:.4f} €/MWh\n", sp_tag(sp))
            w(f"  PTDF ({z_f:<4})    :  {pf:+.4f}\n", 'body_indent')
            w(f"  PTDF ({z_t:<4})    :  {pt:+.4f}\n", 'body_indent')
            w(f"  Delta-PTDF ΔPTDF  :  ", 'body_indent')
            w(f"{dptdf:+.4f}\n", 'val_amber')
            w(f"  Price Impact PI   :  ", 'body_indent')
            w(f"{impact:+.4f} €/MWh", imp_tag(impact))
            w(f"   [{abs(impact / total * 100):.1f}% of total spread]\n\n"
              if total != 0 else "\n\n", 'muted')

            w("Mathematical derivation for this CNEC:\n\n", 'h3')
            w(f"        PI  =  λ · ΔPTDF\n"
              f"            =  λ · (PTDF_{{i,{z_f}}} − PTDF_{{i,{z_t}}})\n"
              f"            =  {sp:.4f}  ×  ({pf:+.4f} − ({pt:+.4f}))\n"
              f"            =  {sp:.4f}  ×  {dptdf:+.4f}\n"
              f"            =  {impact:+.4f} €/MWh\n\n", 'formula')

            w("Physical interpretation:\n", 'h3')
            w(f"  Each additional MWh traded commercially from {z_f} to {z_t}\n"
              f"  changes the flow on this CNEC by {dptdf:+.4f} MW (the\n"
              f"  Delta-PTDF). Since the constraint is priced at {sp:.4f}\n"
              f"  €/MWh of scarcity rent, routing one more MWh across the\n"
              f"  corridor implicitly costs {impact:+.4f} €/MWh in congestion\n"
              f"  rent attributable to this CNEC alone.\n", 'body_indent')

        # ── SECTION 3: CONSTRAINT CHARACTER ASSESSMENT ────────────────
        w("\n\n3.  CONSTRAINT CHARACTER ASSESSMENT\n", 'h2')
        w("─" * 72 + "\n", 'rule')

        if top_sp > 50:
            w("\n  REGIME: ", 'body')
            w(" SYSTEMIC SATURATION ", 'pill_sat')
            w(f"  (λ = {top_sp:.4f} €/MWh)\n\n", 'muted')
            w("  A shadow price above 50 €/MWh signals that the dominant\n"
              "  constraint is not an isolated physical bottleneck — it is\n"
              "  a symptom of systemic capacity scarcity across multiple\n"
              "  correlated elements. In EUPHEMIA's LP relaxation, a high λ\n"
              "  indicates that the constraint's dual feasibility gap is\n"
              "  large: the unconstrained optimum is far inside the infeasible\n"
              "  region. The market is not finding a marginal equilibrium near\n"
              "  the constraint boundary; it is being forced to a structurally\n"
              "  different solution with meaningfully different zone prices.\n\n",
              'body_indent')
            w("  Actionable signal: price spreads in this regime are robust\n"
              "  and unlikely to be arbitraged away intraday unless significant\n"
              "  capacity is released. Cross-zonal trading is heavily rationed\n"
              "  by the flow-based feasible domain.\n", 'muted_indent')

        elif top_sp > 10:
            w("\n  REGIME: ", 'body')
            w(" STRUCTURAL BOTTLENECK ", 'pill_struct')
            w(f"  (λ = {top_sp:.4f} €/MWh)\n\n", 'muted')
            w("  A shadow price in the 10–50 €/MWh band indicates a\n"
              "  well-defined structural constraint whose binding status is\n"
              "  primarily driven by the topology of the Nordic transmission\n"
              "  grid and the current generation dispatch. The constraint\n"
              "  is genuinely limiting cross-zonal capacity, but the market\n"
              "  has enough degrees of freedom to partially compensate — the\n"
              "  spread reflects a real but bounded scarcity rent.\n\n",
              'body_indent')
            w("  Actionable signal: the price spread is structurally\n"
              "  justified and will persist across similar dispatch patterns.\n"
              "  Intraday corrections are possible if generation redispatch\n"
              "  or demand response alters the reference flows.\n", 'muted_indent')

        else:
            w("\n  REGIME: ", 'body')
            w(" SURGICAL BOTTLENECK ", 'pill_surg')
            w(f"  (λ = {top_sp:.4f} €/MWh)\n\n", 'muted')
            w("  A shadow price below 10 €/MWh indicates a localised,\n"
              "  precise constraint: the CNEC is binding, but only marginally.\n"
              "  In EUPHEMIA's KKT conditions, a low λ means the optimum\n"
              "  sits just outside the feasibility set. A small relaxation of\n"
              "  RAM — or a modest shift in reference flows — would render\n"
              "  the constraint non-binding. The resulting price impact is\n"
              "  mathematically real but commercially thin.\n\n",
              'body_indent')
            w("  Actionable signal: the spread is sensitive to small\n"
              "  operational changes (outages, redispatch, wind ramping).\n"
              "  Monitor RAM margins and generation forecasts closely.\n",
              'muted_indent')

        # ── SECTION 4: INDUCED FLOW ANALYSIS ──────────────────────────
        w("\n\n4.  INDUCED FLOW ANALYSIS  (Delta-PTDF)\n", 'h2')
        w("─" * 72 + "\n", 'rule')

        if top:
            cne, czf, czt, sp, pf, pt, impact = top
            dptdf = pf - pt
            induced_100 = dptdf * 100

            w(f"\nThe Delta-PTDF of the dominant CNEC on the {z_f} \u2192 {z_t}\n"
              f"corridor measures the linear flow sensitivity:\n\n", 'body')
            w(f"        ΔPTDF  =  PTDF_{{i,{z_f}}} − PTDF_{{i,{z_t}}}\n"
              f"               =  {pf:+.4f}  −  ({pt:+.4f})\n"
              f"               =  {dptdf:+.4f}\n\n", 'formula')
            w(f"A 100 MW increase in net commercial exports from {z_f} to\n"
              f"{z_t} would load ", 'body')
            w(f"{induced_100:+.2f} MW", sp_tag(abs(induced_100) / 100 * sp))
            w(f" onto this CNEC.\n\n", 'body')

            if dptdf > 0.05:
                w(f"  INTERPRETATION: The CNEC is ")
                w(f"positively coupled ", 'val_red')
                w(f"to the {z_f} \u2192 {z_t}\n"
                  f"  corridor. Every additional MWh of commercial flow in the\n"
                  f"  auction direction loads the constraint further. The CNEC\n"
                  f"  is an active bottleneck for this corridor — it is the\n"
                  f"  physical reason the price spread exists.\n", 'body_indent')
            elif dptdf < -0.05:
                w(f"  INTERPRETATION: The CNEC is ")
                w(f"counter-flow coupled ", 'val_green')
                w(f"to the\n"
                  f"  {z_f} \u2192 {z_t} corridor. Commercial flow in the auction\n"
                  f"  direction actually relieves this constraint. Its positive\n"
                  f"  shadow price must therefore arise from a different\n"
                  f"  directional coupling — examine the biddingZone columns\n"
                  f"  in the Analysis sub-tab for context.\n", 'body_indent')
            else:
                w(f"  INTERPRETATION: The CNEC has near-zero Delta-PTDF on\n"
                  f"  this corridor. Its price impact is geometrically weak —\n"
                  f"  the constraint is binding but electrically decoupled from\n"
                  f"  the {z_f} \u2192 {z_t} exchange. Its contribution to the\n"
                  f"  total spread is minimal regardless of its shadow price.\n",
                  'body_indent')

        # ── SECTION 5: CONSTRAINT LANDSCAPE (top 5) ───────────────────
        w("\n\n5.  CONSTRAINT LANDSCAPE  (top constraints by |impact|)\n", 'h2')
        w("─" * 72 + "\n", 'rule')

        top5 = rows[:min(5, len(rows))]
        hdr  = f"\n  {'#':<3}  {'CNEC':<30}  {'λ (€/MWh)':>10}  {'ΔPTDF':>8}  {'PI (€/MWh)':>11}  {'share':>6}\n"
        w(hdr, 'tbl_head')
        w("  " + "─" * 68 + "\n", 'rule')

        for rank, (cne, czf, czt, sp, pf, pt, impact) in enumerate(top5, 1):
            dptdf   = pf - pt
            share   = f"{abs(impact / total * 100):.1f}%" if total != 0 else "—"
            cne_trunc = (cne[:28] + '..') if len(str(cne)) > 30 else str(cne)
            row_txt = (f"  {rank:<3}  {cne_trunc:<30}  {sp:>10.4f}  "
                       f"{dptdf:>+8.4f}  {impact:>+11.4f}  {share:>6}\n")
            row_tag = 'tbl_row_a' if rank % 2 == 1 else 'tbl_row_b'
            w(row_txt, row_tag)

        remaining = len(rows) - 5
        if remaining > 0:
            w(f"\n  ... and {remaining} further active CNEC(s) with smaller impacts.\n",
              'muted')

        # ── SECTION 6: FORMULA RECONCILIATION ─────────────────────────
        w("\n\n6.  FORMULA RECONCILIATION\n", 'h2')
        w("─" * 72 + "\n", 'rule')

        w("\nVerification that the tabulated individual impacts sum to the\n"
          "reported total — confirming internal consistency:\n\n", 'body')
        running = 0.0
        for i, (cne, czf, czt, sp, pf, pt, impact) in enumerate(rows[:min(5, len(rows))], 1):
            running += impact
            cne_s = (str(cne)[:24] + '..') if len(str(cne)) > 26 else str(cne)
            w(f"        PI_{i}  ({cne_s:<26})  =  {impact:+.4f}\n", 'formula')
        if len(rows) > 5:
            rest = sum(r[6] for r in rows[5:])
            running += rest
            w(f"        PI_n  (remaining {len(rows)-5} CNECs)              =  {rest:+.4f}\n",
              'formula')
        w(f"        {'─'*54}\n", 'rule')
        w(f"        ΔP  (computed sum)                         =  {running:+.4f}\n", 'formula')
        w(f"        ΔP  (reported total)                       =  {total:+.4f}\n", 'formula')

        diff = abs(running - total)
        if diff < 1e-6:
            w(f"\n  Reconciliation: ", 'body')
            w(f"EXACT  (residual < 1e-6 €/MWh)\n", 'val_green')
        else:
            w(f"\n  Reconciliation residual: ", 'body')
            w(f"{diff:.6f} €/MWh  (floating-point rounding)\n", 'val_amber')

        # ── FOOTER ────────────────────────────────────────────────────
        w("\n" + "─" * 72 + "\n", 'rule')
        w("Data source: JAO Publication Tool — Nordic FB Domain Shadow Prices\n"
          "Price impact methodology: SDAC EUPHEMIA documentation, section 4.3\n"
