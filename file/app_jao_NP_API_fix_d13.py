...
class App:
    ...
    def _brfg_write_placeholder(self):
        ...
        w("Run Analysis", 'vb')
        w(". This panel will auto-generate a\nlive intelligence report for that MTU snapshot.\n\n", 'body')

        # Section header helper
        def sec(num, title):
            w(f"  {num}.  {title}  \n", ('h2', 'h2_mark') if num == '' else 'h2')

        # ── Derivation chain — same 6-step structure as live report ──
        sec('', "FLOW-BASED MARKET COUPLING  \u2014  PRICE IMPACT DERIVATION")

        w("\nEUPHEMIA maximises social welfare W subject to DC power-flow and thermal\n"
          "capacity constraints. When a CNEC is binding, its KKT multiplier \u03bb emerges\n"
          "as the scarcity rent that fully explains the zonal price spread.\n\n", 'body')

        w("[1]  EUPHEMIA primal  \u2014  welfare maximisation subject to grid constraints\n", 'fl')
        w("     max  W  =  \u03a3_z [ CS_z(P_z)  +  PS_z(P_z) ]\n"
          "     s.t.\n"
          "          F_n  =  \u03a3_z  PTDF_n,z \u00b7 NP_z          \u2200 n  (DC linearised power flow)\n"
          "          F_n  \u2264  RAM_n                            \u2200 n  (thermal capacity limit)\n"
          "          NP_z \u2208  [NP_z^min,  NP_z^max]           \u2200 z  (zone net-position bounds)\n\n",
          'fm')
        w("     Each binding RAM_n constraint yields a non-zero KKT multiplier \u03bb_n.\n\n", 'fm_c')

        w("[2]  Shadow Price  \u03bb_n  \u2014  KKT dual of the RAM constraint\n", 'fl')
        w("     \u03bb_n  =  \u2202W* / \u2202RAM_n  \u2265  0\n\n", 'fm')
        w("     Loosening CNEC n by 1 MW raises market welfare by \u03bb_n EUR.\n\n", 'fm_c')

        w("[3]  Power Transfer Distribution Factor\n", 'fl')
        w("     PTDF_{n,z}  =  \u2202F_n / \u2202NP_z     (linearised around the reference point)\n\n", 'fm')
        w("     Injecting 1 MW net export from zone z loads PTDF_{n,z} MW onto CNEC n.\n\n", 'fm_c')

        w("[4]  Deriving the zonal price spread from KKT optimality\n", 'fl')
        w("     p_j - p_i  =  \u03a3_n \u03bb_n \u00b7 PTDF_{n,j}  -  ( - \u03a3_n \u03bb_n \u00b7 PTDF_{n,i} )\n\n"
          "     Rearranging:\n\n"
          "     p_j - p_i  =  \u03a3_n \u03bb_n \u00b7 (PTDF_{n,j}  -  PTDF_{n,i})\n\n",
          'fm')

        w("[5]  Bilateral corridor sensitivity  (Delta-PTDF)\n", 'fl')
        w("     \u0394PTDF_n  \u2261  PTDF_{n,ZT}  \u2212  PTDF_{n,ZF}\n\n", 'fm')
        w("     A 1 MW transfer ZF\u2192ZT shifts \u0394PTDF_n MW onto CNEC n.\n\n", 'fm_c')

        w("[6]  Total structural price spread  \u0394P  =  P_ZT \u2212 P_ZF\n", 'fl')
        w("     \u0394P  =  \u03a3_n  \u03bb_n \u00b7 \u0394PTDF_n  =  \u03a3_n  \u03bb_n \u00b7 (PTDF_{n,ZT} \u2212 PTDF_{n,ZF})\n\n", 'fm')
        w("     Complete flow-based explanation of the price spread.\n\n", 'fm_c')

        w("Applied to this MTU\n", 'fl')
        w("     [ Run Analysis to evaluate these equations for your corridor and MTU ]\n\n", 'fm_c')
        w("\u2500" * 72 + "\n", 'doc_rule')

        sec('', "MATHEMATICAL REFERENCE")
    ...
    def _update_briefing_inner(self, rows, total, d_str, t_str, z_f, z_t):
        ...
        # ── FLOW-BASED DERIVATION CHAIN ───────────────────────────────
        w("\nFLOW-BASED MARKET COUPLING  \u2014  PRICE IMPACT DERIVATION\n", 'h2')
        ...
        # [2] Shadow price via KKT
        w("[2]  Shadow Price  \u03bb_n  \u2014  KKT dual of the RAM constraint\n", 'fl')
        w("     \u03bb_n  =  \u2202W* / \u2202RAM_n  \u2265  0\n\n", 'fm')
        w("     Loosening CNEC n by 1 MW raises market welfare by \u03bb_n EUR.\n"
          "     Equivalently, the constraint imposes a scarcity rent of \u03bb_n EUR/MWh\n"
          "     on any flow it limits.  Only binding CNECs have \u03bb_n > 0.\n\n", 'fm_c')

        # [3] PTDF definition
        w("[3]  Power Transfer Distribution Factor\n", 'fl')
        w("     PTDF_{n,z}  =  \u2202F_n / \u2202NP_z     (linearised around the reference point)\n\n", 'fm')
        w("     Injecting 1 MW of net export from zone z loads PTDF_{n,z} MW onto CNEC n.\n\n", 'fm_c')

        # [4]-[5]-[6]  KKT derivation of the price spread — following the academic chain
        w("[4]  Deriving the zonal price spread from KKT optimality\n", 'fl')
        w("     At the optimal solution the zonal prices p_i, p_j satisfy:\n\n"
          "          p_i  =  \u03a3_n  \u03bb_n \u00b7 PTDF_{n,i}       (shadow-price weighted flow cost)\n"
          "          p_j  =  \u03a3_n  \u03bb_n \u00b7 PTDF_{n,j}\n\n"
          "     Subtracting:\n\n"
          "          p_j - p_i  =  \u03a3_n \u03bb_n \u00b7 PTDF_{n,j}  -  ( - \u03a3_n \u03bb_n \u00b7 PTDF_{n,i} )\n\n"
          "     Rearranging:\n\n"
          "          p_j - p_i  =  -\u03a3_n \u03bb_n \u00b7 PTDF_{n,i}  +  \u03a3_n \u03bb_n \u00b7 PTDF_{n,j}\n"
          "                     =   \u03a3_n \u03bb_n \u00b7 (PTDF_{n,j}  -  PTDF_{n,i})\n\n",
          'fm')

        # [5] Delta-PTDF
        w("[5]  Bilateral corridor sensitivity  (Delta-PTDF)\n", 'fl')
        w("     \u0394PTDF_n  \u2261  PTDF_{n,ZT}  \u2212  PTDF_{n,ZF}\n\n", 'fm')
        w("     A 1 MW commercial transfer ZF\u2192ZT changes the flow on CNEC n by \u0394PTDF_n MW.\n"
          "     Positive \u0394PTDF_n means the transfer loads the constraint toward its RAM limit.\n\n", 'fm_c')

        # [6] Total spread — the price impact formula
        w("[6]  Total structural price spread  \u0394P  =  P_ZT \u2212 P_ZF\n", 'fl')
        w("     \u0394P  =  \u03a3_n  \u03bb_n \u00b7 \u0394PTDF_n  =  \u03a3_n  \u03bb_n \u00b7 (PTDF_{n,ZT} \u2212 PTDF_{n,ZF})\n\n", 'fm')
        w("     This is the complete flow-based explanation of the zonal price spread.\n"
          "     Each binding CNEC contributes  PI_n = \u03bb_n \u00b7 \u0394PTDF_n  EUR/MWh.\n"
          "     The sum across all active CNECs equals the price differential observed\n"
          "     between the two bidding zones in the day-ahead auction.\n\n", 'fm_c')

        # Evaluated result for this MTU
        w("Applied to this MTU\n", 'fl')
        if n_active == 0:
            w("     No binding CNECs  \u2014  \u0394P = 0,  corridor is uncongested.\n\n", 'fm_c')
        else:
            cne0, czf0, czt0, sp0, pf0, pt0, pi0 = rows[0]
            dp0   = pf0 - pt0
            share = abs(pi0 / total * 100) if total else 0.0
            w(f"     \u0394P  =  \u03a3_n \u03bb_n \u00b7 \u0394PTDF_n\n\n", 'fm')
            w(f"          Dominant term  (n = {str(cne0)[:40]}):\n", 'fm_c')
            w(f"          PI_1  =  \u03bb_1 \u00b7 (PTDF_{{ZT}} \u2212 PTDF_{{ZF}})\n"
              f"               =  {sp0:.4f}  \u00d7  ({pf0:+.4f} \u2212 {pt0:+.4f})\n"
              f"               =  {sp0:.4f}  \u00d7  {dp0:+.4f}\n"
              f"               =  {pi0:+.4f} \u20ac/MWh    ({share:.1f}% of \u0394P)\n\n", 'fm')
            w(f"          Summing all {n_active} active CNECs:\n", 'fm_c')
            w(f"          \u0394P  =  {total:+.4f} \u20ac/MWh\n\n", 'fm')
            spread_dir = (f"{z_t} clears {abs(total):.4f} \u20ac/MWh above {z_f}"
                          if total >= 0
                          else f"{z_f} clears {abs(total):.4f} \u20ac/MWh above {z_t}")
            w(f"          \u2192  {spread_dir}, as required by the flow-based feasibility domain.\n",
              'fm_c')
    ...
    def _build_briefing_pdf(self, path, b):
        ...
        def sty(name, **kw):
            base = ParagraphStyle(name,
                                  ...
                                  textColor=kw.pop('textColor', SLATE),
